...
messaging.setBackgroundMessageHandler(function(payload) {
  ...
  const notificationTitle = 'Moveleiros Chat';
  const notificationOptions = {
    body: 'Novo chat moveleiros',
    icon: '/static/img/launcher-icon-4x.png'
  };

  self.clients.matchAll().then(function (clients) {
    clients.forEach(c => { c.postMessage('saporra') })
  });
...
self.addEventListener('activate', function(e) {
  ...
  console.log('saporra', self, e);
});

self.addEventListener('push', function(event) {
  console.log("Broadcastinghe clients", self);
  ...
  var promise = clients.matchAll({
    ...
    for (var i = 0; i < clientList.length; i++) {
      ...
      if ('focus' in client) {
        console.log("Possui focus", self);
        client.focus()
        client.postMessage('saporra')
        return
      }
    }
